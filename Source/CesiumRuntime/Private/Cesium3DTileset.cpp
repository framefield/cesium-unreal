...
void ACesium3DTileset::OnFocusEditorViewportOnThis()
{
  ...
  struct CalculateECEFCameraPosition
  {
    ...
    operator()(const CesiumGeometry::OrientedBoundingBox& orientedBoundingBox)
    {
      ...
      glm::dvec3 offset =
        length(halfAxes[0] + halfAxes[1] + halfAxes[2]) *
        normalize(
          glm::dvec3(ENU[0]) + glm::dvec3(ENU[1]) + glm::dvec3(ENU[2]));
      glm::dvec3 position = center + offset;
      return position;
...
public:
    ...
    const std::any& rendererOptions) override
  {
    ...
    auto texture = loadTextureAnyThreadPart(
    ...
    void* pLoadThreadResult) override
  {
    ...
    UTexture2D* pTexture =
      loadTextureGameThreadPart(pLoadedTexture.Get());
    if (!pTexture)
    {
      return nullptr;
    }
    ...
    void* pMainThreadResult) noexcept override
  {
    if (pLoadThreadResult)
    {
      CesiumTextureUtility::LoadedTextureResult* pLoadedTexture =
        static_cast<CesiumTextureUtility::LoadedTextureResult*>(
          pLoadThreadResult);
      destroyHalfLoadedTexture(*pLoadedTexture);
...
void ACesium3DTileset::LoadTileset()
{
  ...
  options.loadErrorCallback =
    ...
    {
      static_assert(
        static_cast<uint8_t>(ECesium3DTilesetLoadType::CesiumIon) ==
        static_cast<uint8_t>(Cesium3DTilesSelection::TilesetLoadType::CesiumIon));
      static_assert(
        static_cast<uint8_t>(ECesium3DTilesetLoadType::TilesetJson) ==
        static_cast<uint8_t>(Cesium3DTilesSelection::TilesetLoadType::TilesetJson));
      static_assert(
        static_cast<uint8_t>(ECesium3DTilesetLoadType::Unknown) ==
        static_cast<uint8_t>(Cesium3DTilesSelection::TilesetLoadType::Unknown));

      uint8_t typeValue = static_cast<uint8_t>(details.type);
      assert(
      ...
      ueDetails.Type = static_cast<ECesium3DTilesetLoadType>(typeValue);
  ...
  supportedFormats.ETC1_RGB = GPixelFormats[PF_ETC1].Supported;
  supportedFormats.ETC2_RGBA =
    GPixelFormats[PF_ETC2_RGBA].Supported;
  supportedFormats.BC1_RGB = GPixelFormats[PF_DXT1].Supported;
  supportedFormats.BC3_RGBA = GPixelFormats[PF_DXT5].Supported;
  supportedFormats.BC4_R = GPixelFormats[PF_BC4].Supported;
  supportedFormats.BC5_RG = GPixelFormats[PF_BC5].Supported;
  supportedFormats.BC7_RGBA = GPixelFormats[PF_BC7].Supported;
  ...
  supportedFormats.ETC2_EAC_R11 =
    GPixelFormats[PF_ETC2_R11_EAC].Supported;
  supportedFormats.ETC2_EAC_RG11 =
    GPixelFormats[PF_ETC2_RG11_EAC].Supported;
...
std::vector<FCesiumCamera> ACesium3DTileset::GetPlayerCameras() const
{
       ...
       ++playerControllerIt)
  {
    ...
    if (useStereoRendering)
    {
      ...
      constexpr auto leftEye = eSSE_LEFT_EYE;
      constexpr auto rightEye = eSSE_RIGHT_EYE;
#else
  ...
  const glm::dmat4& unrealWorldToTileset)
{
  ...
  glm::dvec3 tilesetCameraFront = normalize(glm::dvec3(
    unrealWorldToTileset *
    glm::dvec4(direction.X, direction.Y, direction.Z, 0.0)));
  glm::dvec3 tilesetCameraUp = normalize(
  ...
  const std::vector<Cesium3DTilesSelection::Tile*>& tiles)
{
  ...
  for (Cesium3DTilesSelection::Tile* pTile : tiles)
  {
...

    {
      ...
      Gltf->SetCollisionEnabled(this->CreatePhysicsMeshes
...
void ACesium3DTileset::Tick(float DeltaTime)
{
  ...
  glm::dmat4 unrealWorldToCesiumTileset =
    affineInverse(ueTilesetToUeWorld * cesiumTilesetToUeTileset);
  ...
  if (EvaluateCustomTileCulling || EvaluateTileCullingIntersection)
  {
    ...
    std::unordered_set<Cesium3DTilesSelection::Tile*> ChangedTilesFadingOut = pResult->tilesFadingOut;
    auto TilesCulled = pResult->tilesCulled;
    ...
    for (const auto Tile : pResult->tilesToRenderThisFrame)
    {
      if (Tile != nullptr)
      {
        bool IsCulled = false;
    ...
    // Oh no! We have to cast away constness here, because the API doesn't support modifying the result anymore
    const auto MutableResult = const_cast<Cesium3DTilesSelection::ViewUpdateResult*>(pResult);
    MutableResult->tilesToRenderThisFrame = ChangedRenderEntries;
    MutableResult->tilesFadingOut = ChangedTilesFadingOut;
    MutableResult->tilesCulled = TilesCulled;
  }
  /// END FF CHANGES
